...
if ((x[0]==dotX  )&&(y[0]==dotY)){
    ...
    public void checkCollisions(){
        //daca capul atinge corpul?? (cause)
for (int i = body; i>0; i--){
    if ((x[0]==x[i])&&(y[0]==y[i])){
        running=false;
    }
}
//  daca capul atinge partea stanga ?? (cause))
        if (x[0]<0){
            running=false;
        }
        // daca capul atinge partea dreapta ??(cause)
        if (x[0]>SCREEN_WIDTH){
            running=false;
        }
        // daca capul atinge partea de sus ??(cause)
        if (y[0]<0){
            running=false;
        }
        //daca capul atinge partea de jos ?? (cause)
